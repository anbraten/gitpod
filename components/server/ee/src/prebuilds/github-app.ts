...
export class GithubApp {
    ...
    protected async handlePushEvent(ctx: Context<'push'>): Promise<void> {
        ...
        try {
            ...
            let owner = installationId && (await this.findProjectOwner(cloneURL) || (await this.findInstallationOwner(installationId)));
            if (!owner) {
            ...
            if (!!context.subRepositoryCheckoutInfo && (!owner.project || context.repository.cloneUrl !== owner.project.cloneUrl)) {
                owner = await this.findProjectOwner(context.repository.cloneUrl);
                if (!owner) {
